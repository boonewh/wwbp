...
import PlanningReference,{SourceBudget,DestinationInfo} from './planning-reference';
...
export default function OrderEditor({campaign,report,atlas,busy,onSave,onDirty,latestTurn}:{campaign:Campaign;report:Report;atlas:Record<string,Geography>;busy:boolean;onSave:(draft:OrderDraft)=>Promise<void>;onDirty:(dirty:boolean)=>void;latestTurn:number}){
 ...
 function chooseSource(code:string){setSource(code);if(builder.current)builder.current.open=true;}
 function chooseTarget(code:string){setTarget(code);if(builder.current)builder.current.open=true;}
 function append(){
 ...
 return <section className="order-editor" aria-labelledby="orders-heading">
  ...
  <div className="planning-layout"><PlanningReference report={report} atlas={atlas} source={source} onSource={chooseSource} onTarget={chooseTarget}/><div className="planning-editor-pane" role="region" aria-label="Draft and order builder" tabIndex={0}>
   <details open ref={builder} className="order-builder"><summary>Add a space order</summary><div className="order-builder-fields">
   ...
   </div><SourceBudget source={source} report={report} budgets={result.budgets}/>{!build&&<DestinationInfo code={target} report={report} atlas={atlas}/>}<BuilderHelp source={source} action={action} amount={amount} target={target} atlas={atlas}/><button type="button" disabled={!source||!/^\d+$/.test(amount)||+amount<1||!build&&!atlas[target]} onClick={append}>Add order</button></details>
   <label className="order-label" htmlFor="order-text">Your orders</label><p id="order-help">Edit your draft or paste orders here. Start player orders with @ and each space with @ plus its code. ORDERS and END are optional here; export adds them. Keep your email heading, security code, and messages out of this box.</p>
...
<section className="planning-checks" aria-label="Order checks"><h3>Checks</h3><p>{result.count} orders · {errors} errors · {warnings} warnings</p><p>Context markers (@) do not count as orders.</p>{!text.trim()?<p>Start a draft to see explanations and checks.</p>:errors===0&&<p>No errors found in the rules checked below.</p>}
...
<ul className="planning-issues">{result.rows.flatMap((row,i)=>row.issues.map((issue,j)=><li key={i+'-'+j} className={'order-'+issue.level}><button type="button" className="text-button" onClick={()=>locate(row.line)}>Line {row.line} · {row.token}</button>: {issue.message}</li>))}</ul>
   <details><summary>Checker coverage, defaults, and standing orders</summary><h3>What is checked</h3><p>Full order syntax, source control, space codes, unit range, minor restrictions, explicit resource totals, duplicate space orders, and common attack/support conflicts.</p>
